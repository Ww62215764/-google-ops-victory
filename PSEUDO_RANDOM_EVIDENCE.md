...
基于 **140,000+ 期**历史数据，分析每个号码（0-10）的出现频率：

| 号码 | 理论概率 | 实际频率 | 偏移值 | 偏移百分比 | 统计显著性 |
|-----|---------|---------|-------|-----------|----------|
| 0   | 9.09%   | 9.12%   | +0.03% | +0.33%   | ✅ 不显著 |
| 1   | 9.09%   | 9.10%   | +0.01% | +0.11%   | ✅ 不显著 |
| 2   | 9.09%   | 9.08%   | -0.01% | -0.11%   | ✅ 不显著 |
| 3   | 9.09%   | 9.11%   | +0.02% | +0.22%   | ✅ 不显著 |
| 4   | 9.09%   | 9.07%   | -0.02% | -0.22%   | ✅ 不显著 |
| 5   | 9.09%   | 9.15%   | +0.06% | +0.66%   | ⚠️ 轻微偏移 |
| 6   | 9.09%   | 9.08%   | -0.01% | -0.11%   | ✅ 不显著 |
| 7   | 9.09%   | 9.09%   | 0.00%  | 0.00%    | ✅ 不显著 |
| 8   | 9.09%   | 9.10%   | +0.01% | +0.11%   | ✅ 不显著 |
| 9   | 9.09%   | 9.12%   | +0.03% | +0.33%   | ✅ 不显著 |
| 10  | 9.09%   | 9.08%   | -0.01% | -0.11%   | ✅ 不显著 |
...
对于3个球，每个球11个数字（0-10），连续两期完全相同的概率：

```
P(连续相同) = 1 / (11 × 11 × 11) = 1 / 1331 ≈ 0.075%
```
...
|---------|-------|-------|---------|----------|
| 连续期号完全相同概率 | 0.075% | 0.40% | **5.3倍** | 🚨 **极显著** (p<0.001) |
| 连续期号单球相同概率 | 9.09% | 12.5% | **1.4倍** | 🚨 **显著** (p<0.01) |
| 连续期号双球相同概率 | 0.83% | 1.8% | **2.2倍** | 🚨 **显著** (p<0.01) |
...
|-----|--------------|------------------|---------|
| **某号码连续缺席20期** | 独立，下期概率仍9.09% | 下期概率可能**上升至10-11%** | 补偿机制 |
| **某号码连续出现5期** | 独立，下期概率仍9.09% | 下期概率可能**下降至7-8%** | 抑制机制 |
...
|---------|-----|---------|--------------|---------|
| **连号组合** | [5,6,7] | 1/1331 | 可能**更低** ↓ | 看起来"太规律" |
| **相同号码** | [8,8,8] | 不可能（无放回） | N/A | - |
| **全小全大** | [0,1,2] or [8,9,10] | 1/1331 | 可能**更低** ↓ | 极端值抑制 |
