...
# This must be after standard and third-party imports
...
class MonitoringMiddleware(BaseHTTPMiddleware):
    async def dispatch(
        ...
        try:
            ...
            background.add_task(self.send_metrics, request.url.path, request.method, status_code, latency_ms)
            if response:
...
def call_api_with_retry(api_url: str, params: dict, api_key: str) -> dict:
    ...
    response = requests.get(api_url, params=params, timeout=10, headers={"User-Agent": "DrawsGuard/7.0"})
...
async def collect(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if current_period_str:
            detect_and_handle_upstream_stale(
                collector_name="pc28_main_api", returned_period=int(current_period_str), response_json=json.dumps(data)
            )
        ...
        raise HTTPException(status_code=429, detail=f"Upstream is stale, halting requests: {e}") from e
    except Exception as e:
